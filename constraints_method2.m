...
for i = 1:1:size(playersPairs,1)
    ...
    if intersectionBetweenEnvelopesOfPlayers(i) > 0
        ...
        for k = 1:1:players{playersPairs(i,1)}.opt.params.N+1
            ...
            tempconstraints = [tempconstraints; sigma_afp(k)];
            templb = [templb; -inf];
            tempub = [tempub; h];
            
            tempconstraints = [tempconstraints; sigma_bep(k)];
            templb = [templb; -inf];
            tempub = [tempub; h];
            
            tempconstraints = [tempconstraints; sigma_cdp(k)];
            templb = [templb; -inf];
            tempub = [tempub; h];
            ...
            tempconstraints = [tempconstraints; sigma_afp(k)];
            templb = [templb; -inf];
            tempub = [tempub; sigma_af(k)];
            
            tempconstraints = [tempconstraints; sigma_bep(k)];
            templb = [templb; -inf];
            tempub = [tempub; sigma_be(k)];
            
            tempconstraints = [tempconstraints; sigma_cdp(k)];
            templb = [templb; -inf];
            tempub = [tempub; sigma_cd(k)];

            %%%
            tempconstraints = [tempconstraints; sigma_afp(k)];
            tempub = [tempub; inf];
            templb = [templb; sigma_af(k) - 1 + h];
            
            tempconstraints = [tempconstraints; sigma_bep(k)];
            tempub = [tempub; inf];
            templb = [templb; sigma_be(k) - 1 + h];
            
            tempconstraints = [tempconstraints; sigma_cdp(k)];
            tempub = [tempub; inf];
            templb = [templb; sigma_cd(k) - 1 + h];

%%
            % 17a
            tempconstraints = [tempconstraints; x_2(k,1) + bigM*sigma_af(k) - bigM*sigma_afp(k)];
            tempub = [tempub; s2_l_entry{i} + bigM];
            templb = [templb; -inf];

            % 17b
            tempconstraints = [tempconstraints; x_2(k,1) - x_1(k,1) + bigM*sigma_be(k) - bigM*sigma_bep(k)];
            tempub = [tempub; bigM + (s2_l_entry{i} - s1_h_entry{i})];
            templb = [templb; -inf];
            
            % 17c
            tempconstraints = [tempconstraints; x_1(k,1) - bigM*sigma_cd(k) + bigM*sigma_cdp(k)];
            tempub = [tempub; inf];
            templb = [templb; s1_h_exit{i} - bigM];
                        
            % 17d
            tempconstraints = [tempconstraints; x_1(k,1) + bigM*sigma_cdp(k)];
            tempub = [tempub; s1_l_entry{i} + bigM];
            templb = [templb; -inf];
 
            % 17e
            tempconstraints = [tempconstraints; x_2(k,1) - x_1(k,1) - bigM*sigma_bep(k)];
            tempub = [tempub; inf];
            templb = [templb; (s2_h_entry{i} - s1_l_entry{i}) - bigM];

            % 17f
            tempconstraints = [tempconstraints; x_2(k,1) - bigM*sigma_afp(k)];
            tempub = [tempub; inf];
            ...
            if k > 1
                ...
                % 17a
                tempconstraints = [tempconstraints; x_2(k-1,1) + bigM*sigma_af(k) - bigM*sigma_afp(k)];
                tempub = [tempub; s2_l_entry{i} + bigM];
                templb = [templb; -inf];
    
                % 17b
                tempconstraints = [tempconstraints; x_2(k-1,1) - x_1(k-1,1) + bigM*sigma_be(k) - bigM*sigma_bep(k)];
                tempub = [tempub; bigM + (s2_l_entry{i} - s1_h_entry{i})];
                templb = [templb; -inf];
                
                % 17c
                tempconstraints = [tempconstraints; x_1(k-1,1) - bigM*sigma_cd(k) + bigM*sigma_cdp(k)];
                tempub = [tempub; inf];
                templb = [templb; s1_h_exit{i} - bigM];
                            
                % 17d
                tempconstraints = [tempconstraints; x_1(k-1,1) + bigM*sigma_cdp(k)];
                tempub = [tempub; s1_l_entry{i} + bigM];
                templb = [templb; -inf];
     
                % 17e
                tempconstraints = [tempconstraints; x_2(k-1,1) - x_1(k-1,1) - bigM*sigma_bep(k)];
                tempub = [tempub; inf];
                templb = [templb; (s2_h_entry{i} - s1_l_entry{i}) - bigM];
    
                % 17f
                tempconstraints = [tempconstraints; x_2(k-1,1) - bigM*sigma_afp(k)];
                tempub = [tempub; inf];
                templb = [templb; s2_h_exit{i} - bigM];
...
%                 end
                ...
                for k = 1:1:length(sigmaOfInterest_2_1)
                    ...
                    temp_lb = [temp_lb; 1 - bigM*(1-sigmaOfInterest(k))];
                    temp_ub = [temp_ub; 1 + bigM*(1-sigmaOfInterest(k))];              
                end
