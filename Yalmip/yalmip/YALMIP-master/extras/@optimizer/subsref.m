    ...
    for i = 1:nBlocks
        ...
        if ~self.model.infeasible
            if self.model.options.warmstart && ~isempty(self.lastsolution)
                ...
                self.model.x0 = self.lastsolution; % this is where the last solution gets assigned to the 
                                                   % initial guess before it is passed to the solver
            ...
            if NoSolve
                ...
                eval(['output = ' self.model.solver.call '(self.model,self.map,self.psdims,self.dimoutOrig);']);
            end
            % here is the place to modify the last solution to make it seem
            % like MPC. Moved to buttom 
%             if output.problem == 0 && self.model.options.warmstart
%                 self.lastsolution = output.Primal;
%          %       aaLastSolution = self.lastsolution;
%             end
...
if output.problem == 0 && self.model.options.warmstart
    ...
    psdims_N = self.psdims(1);
    psdims_n = self.psdims(2);

    %%%%%
    temp1 = reshape(allu{1},[psdims_N+1 length(allu{1})/(psdims_N+1)]);
    temp1 = temp1(2:end,:);
    %temp1 =  [temp1;zeros(1,length(allu{1})/(psdims_N+1))];
    temp1 = [temp1;nan+temp1(end,:)];
    %%%%%%

    temp2 = reshape(allu{2},[psdims_N length(allu{2})/(psdims_N)]);
    temp2 = temp2(2:end,:);
    %temp2 =  [temp2;zeros(1,length(allu{2})/(psdims_N))];
    temp2 = [temp2;nan+temp2(end,:)];
    %%%%%%

    temp3 = reshape(allu{4},[psdims_N+1 length(allu{4})/(psdims_N+1)]);
    temp3 = temp3(2:end,:);
    %temp3 =  [temp3;zeros(1,length(allu{4})/(psdims_N+1))];
    temp3 = [temp3;nan+temp3(end,:)];
    %%%%%%%%%%%%

    allu_vec = [temp1(:);temp2(:);allu{3};temp3(:);allu{5}];
    
    tempVec = allu_vec(mapc_solver);
    tempVec(self.model.removethese) = [];
    
    self.lastsolution = tempVec;
end
% if norm(tempVec-self.lastsolution) > 0
%     norm(tempVec-self.lastsolution)
% else
%     sprintf('everthing ok')
% end
    varargout{5} = self;
        varargout{6} = output;
end
