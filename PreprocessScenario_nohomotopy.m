...
% Constraints Method 3 setups up without using the notion of homotopies
constraints_method3()
...
for i = 1:1:size(playersPairs,1)
    ...
    if intersectionBetweenEnvelopesOfPlayers(i) > 0
        ...
        homotopy_vars = [homotopy_vars; nohomotopy_setup.constraints.vars.constraints{i}(:)];
        homotopy_vars_ub = [homotopy_vars_ub; nohomotopy_setup.constraints.vars.ub{i}(:) ];
        homotopy_vars_lb = [homotopy_vars_lb; nohomotopy_setup.constraints.vars.lb{i}(:)];

        homotopy_constraints = [homotopy_constraints; nohomotopy_setup.constraints.general.constraints{i}(:) ];
        homotopy_constraints_ub = [homotopy_constraints_ub; nohomotopy_setup.constraints.general.ub{i}(:)];
        homotopy_constraints_lb = [homotopy_constraints_lb; nohomotopy_setup.constraints.general.lb{i}(:)];

        tempGuess_homotopy = zeros(length(nohomotopy_setup.constraints.vars.constraints{i}(:)),1);
